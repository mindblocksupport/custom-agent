...
"""
    ...
    Detect --> Reorder["LongContextReorder"]
...
nav.toc > h2 {  /* TOC 标题 — 不要继承 main 的 h2 样式 */
  font-size: 13px;
  margin: 0 0 14px;
  padding: 0;
...
li > ul, li > ol { margin: 6px 0; }  /* 嵌套列表更紧凑 */
code {
...
.pron:hover .ipa { display: block; }   /* 悬停显示音标 (浮在上方) */
.pron:hover .speak-btn { display: inline; }  /* 悬停显示喇叭 */
...
function _doSpeak(word) {
  try {
    ...
    u.onerror = (e) => {
      ...
      alert('语音播放出错: ' + e.error + ' (词: ' + word + ')\n\n常见原因:\n1. 浏览器禁用了 Web Speech (devtools console 看详情)\n2. 系统 TTS 引擎未启用\n3. 用户未与页面交互 (Chrome 要求点击后才允许)');
    };
